...
import org.idnecology.idn.tests.acceptance.dsl.transaction.SignUtil;
import org.idnecology.idn.tests.acceptance.dsl.transaction.account.TransferTransaction;
...
public class PermissioningPluginTest extends AcceptanceTestBase {
  ...
  private IdnNode aliceNode;
  private IdnNode bobNode;
  private IdnNode charlieNode;

  private static final long GAS_LIMIT_THRESHOLD = 22000L;

  @BeforeEach
  public void setUp() throws Exception {
    minerNode = idn.create(createNodeBuilder().name("miner").build());

    aliceNode = idn.create(createNodeBuilder().name("alice").keyFilePath("key").build());

    bobNode = idn.create(createNodeBuilder().name("bob").keyFilePath("key1").build());

    charlieNode = idn.create(createNodeBuilder().name("charlie").keyFilePath("key2").build());
  ...
  private IdnNodeConfigurationBuilder createNodeBuilder() {
