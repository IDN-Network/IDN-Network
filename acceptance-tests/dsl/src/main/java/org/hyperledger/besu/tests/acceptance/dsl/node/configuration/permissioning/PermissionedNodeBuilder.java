 ...
 */
package org.idnecology.idn.tests.acceptance.dsl.node.configuration.permissioning;
...
import org.idnecology.idn.ethereum.permissioning.AllowlistPersistor.ALLOWLIST_TYPE;
...
public class PermissionedNodeBuilder {
  ...
  public IdnNode build() {
    ...
    final IdnNodeConfigurationBuilder builder = new IdnNodeConfigurationBuilder();
    builder
        .name(name)
    ...
    try {
      return new IdnNodeFactory().create(builder.build());
    } catch (IOException e) {
      throw new RuntimeException("Error creating IdnNode", e);
    }
  }
