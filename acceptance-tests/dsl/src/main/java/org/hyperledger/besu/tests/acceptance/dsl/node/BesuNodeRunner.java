...
public interface IdnNodeRunner {

  static IdnNodeRunner instance() {
    if (isProcessIdnNodeRunner()) {
      return new ProcessIdnNodeRunner();
    } else {
      return new ThreadIdnNodeRunner();
    }
  }

  static boolean isProcessIdnNodeRunner() {
    return Boolean.getBoolean("acctests.runIdnAsProcess");
  }

  void startNode(IdnNode node);

  void stopNode(IdnNode node);
