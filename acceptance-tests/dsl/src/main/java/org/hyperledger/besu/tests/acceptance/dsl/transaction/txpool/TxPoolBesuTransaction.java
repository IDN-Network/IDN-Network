 ...
 */
package org.idnecology.idn.tests.acceptance.dsl.transaction.txpool;
...
public class TxPoolIdnTransaction implements Transaction<List<Map<String, String>>> {

  @Override
  public List<Map<String, String>> execute(final NodeRequests node) {
    try {
      return node.txPool().idnTransactions().send().getResult();
    } catch (IOException e) {
