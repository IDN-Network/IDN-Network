...
/**
 * Base interface for Idn plugins.
 *
 * <p>Plugins are discovered and loaded using {@link java.util.ServiceLoader} from jar files within
 * Idn's plugin directory. See the {@link java.util.ServiceLoader} documentation for how to
 * register plugins.
 */
public interface IdnPlugin {
  ...
  /**
   * Called when the plugin is first registered with Idn. Plugins are registered very early in the
   * Idn life-cycle and should use this callback to register any command line options required via
   * the PicoCLIOptions service.
   *
   * <p>The <code>context</code> parameter should be stored in a field in the plugin. This is the
   * only time it will be provided to the plugin and is how the plugin will interact with Idn.
   *
   * <p>Typically the plugin will not begin operation until the {@link #start()} method is called.
   *
   * @param context the context that provides access to Idn services.
   */
  void register(ServiceManager context);

  /**
   * Called once when idn has loaded configuration but before external services have been started
   * e.g. metrics and http
   */
  default void beforeExternalServices() {}

  /**
   * Called once Idn has loaded configuration and has started external services but before the main
   * loop is up. The plugin should begin operation, including registering any event listener with
   * Idn services and starting any background threads the plugin requires.
   */
  void start();
  ...
  /**
   * Called when the plugin is being stopped. This method will be called as part of Idn shutting
