...
# Run Idn first to get paths needing permission adjustment
output=$(/opt/idn/bin/idn --print-paths-and-exit $BESU_USER_NAME "$@")
...
echo "$output" | while IFS=: read -r prefix path accessType; do
    if [[ "$prefix" == "PERMISSION_CHECK_PATH" ]]; then
      ...
      if [[ "$accessType" == "READ" ]]; then
        ...
        # Set read/write permissions for idn user
        # Add execute for directories to allow access
...
# Switch to the idn user and execute the command
