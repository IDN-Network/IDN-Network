 ...
 */
package org.idnecology.idn.cli.subcommands.storage;

import static com.google.common.base.Preconditions.checkArgument;
import static com.google.common.base.Preconditions.checkNotNull;
import static org.idnecology.idn.cli.subcommands.storage.RocksDbHelper.formatOutputSize;
import static org.idnecology.idn.controller.IdnController.DATABASE_PATH;
import static org.idnecology.idn.ethereum.storage.keyvalue.KeyValueSegmentIdentifier.TRIE_LOG_STORAGE;
...
public class TrieLogSubCommand implements Runnable {
  ...
  private static IdnController createIdnController() {
    final DataStorageConfiguration config = parentCommand.idnCommand.getDataStorageConfiguration();
  ...
  static class PruneTrieLog implements Runnable {
    ...
    public void run() {
      ...
      final Path dataDirectoryPath =
          Paths.get(
              TrieLogSubCommand.parentCommand.idnCommand.dataDir().toAbsolutePath().toString());
  ...
  @Command(
      name = "import",
      description = "This command imports a trie log exported by another idn node",
      mixinStandardHelpOptions = true,
      versionProvider = VersionProvider.class)
  ...
  private static TrieLogContext getTrieLogContext() {
    ...
    IdnController idnController = createIdnController();
    final DataStorageConfiguration config = idnController.getDataStorageConfiguration();
    checkArgument(
        DataStorageFormat.BONSAI.equals(config.getDataStorageFormat()),
        "Subcommand only works with data-storage-format=BONSAI");

    final StorageProvider storageProvider = idnController.getStorageProvider();
    final BonsaiWorldStateKeyValueStorage rootWorldStateStorage =
        (BonsaiWorldStateKeyValueStorage) storageProvider.createWorldStateStorage(config);
    final MutableBlockchain blockchain = idnController.getProtocolContext().getBlockchain();
