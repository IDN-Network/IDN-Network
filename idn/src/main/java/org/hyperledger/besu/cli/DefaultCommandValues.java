...
public interface DefaultCommandValues {
  ...
  String BESU_HOME_PROPERTY_NAME = "idn.home";
  ...
  /**
   * Gets default idn data path.
   *
   * @param command the command
   * @return the default idn data path
   */
  static Path getDefaultIdnDataPath(final Object command) {
    // this property is retrieved from Gradle tasks or Idn running shell script.
    final String idnHomeProperty = System.getProperty(BESU_HOME_PROPERTY_NAME);
    final Path idnHome;

    // If prop is found, then use it
    if (idnHomeProperty != null) {
      try {
        idnHome = Paths.get(idnHomeProperty);
    ...
    try {
      Files.createDirectories(idnHome);
    } catch (final FileAlreadyExistsException e) {
      // Only thrown if it exists but is not a directory
      throw new CommandLine.ParameterException(
          new CommandLine(command),
          String.format("%s: already exists and is not a directory.", idnHome.toAbsolutePath()),
          e);
    } catch (final Exception e) {
      throw new CommandLine.ParameterException(
          new CommandLine(command),
          String.format("Error creating directory %s.", idnHome.toAbsolutePath()),
          e);
    }
    return idnHome;
  }
