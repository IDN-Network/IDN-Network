...
public abstract class AbstractCLIOptionsTest<D, T extends CLIOptions<D>>
  ...
  private void getCLIOptions(final D domainObject) {
    ...
    final TestIdnCommand cmd = parseCommand(cliOptions);
    final T optionsFromCommand = getOptionsFromIdnCommand(cmd);
  ...
  protected abstract T getOptionsFromIdnCommand(final TestIdnCommand idnCommand);
  ...
  protected void internalTestSuccess(
      final BiFunction<TestIdnCommand, D, D> runtimeConf,
      final Consumer<D> assertion,
      final String... args) {
    final TestIdnCommand cmd = parseCommand(args);

    final T options = getOptionsFromIdnCommand(cmd);
